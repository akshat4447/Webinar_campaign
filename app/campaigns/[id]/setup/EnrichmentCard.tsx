  ...
  return (
    <div style={{ background: '#fff', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-card)', padding: '18px 20px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: 12 }}>
        <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--n90)' }}>Enrichment</div>
      ...
      {stats.total === 0 ? (
        <div style={{ fontSize: 12.5, color: 'var(--n60)' }}>Import contacts first — enrichment fills the gaps in whatever the source data is missing.</div>
      ) : (
        <>
          ...
          <div style={{ fontSize: 12, color: 'var(--n60)', lineHeight: 1.55, marginBottom: 14 }}>
          ...
          {result?.ok && (
            <div style={{ fontSize: 12.5, color: 'var(--success-700)', lineHeight: 1.6, marginBottom: 12 }}>
          ...
          {result && !result.ok && <div style={{ fontSize: 12, color: 'var(--danger-500)', marginBottom: 12 }}>{result.error}</div>}
...
function Stat({ label, value, warn }: { label: string; value: number; warn?: boolean }) {
  return (
    <div>
      <div style={{ fontSize: 10.5, fontWeight: 600, color: 'var(--n60)', textTransform: 'uppercase', letterSpacing: '0.03em' }}>{label}</div>
      <div style={{ fontSize: 18, fontWeight: 700, color: warn ? 'var(--warning-700)' : 'var(--n90)', marginTop: 2 }}>{value}</div>
    </div>
  );
}
