...
export function RunScoringPrompt({ campaignId, contactCount }: { campaignId: string; contactCount: number }) {
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-card)', padding: '32px 28px', textAlign: 'center', maxWidth: 480, margin: '48px auto' }}>
      <div style={{ width: 40, height: 40, borderRadius: '50%', background: 'var(--accent-purple)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 16px', fontSize: 13, fontWeight: 700, color: '#fff' }}>
        C
      </div>
      <div style={{ fontSize: 15, fontWeight: 700, color: 'var(--n90)', marginBottom: 8 }}>Ready to score {contactCount} contacts</div>
      <div style={{ fontSize: 13, color: 'var(--n60)', lineHeight: 1.55, marginBottom: 20 }}>
      ...
      {error && <div style={{ fontSize: 12.5, color: 'var(--danger-500)', marginBottom: 12 }}>{error}</div>}
